...
function update_pmf_z1!(hs::HiddenStates{M}, mp::ModelParameters{N, M}, x_seq) where {N, M}
    mp.pmf_z1 = hs.z_ests[1] / sum(hs.z_ests[1])
end
...
function update_prop_list!(hs::HiddenStates{M}, mp::ModelParameters{N, M}, x_seq) where {N, M}
    for i in 1:M
        w_seq = [z_est[i] for z_est in hs.z_ests] # TODO generator
        fit!(mp.prop_list[i], x_seq, w_seq)
    end
end

function update_model_parameters!(hs::HiddenStates{M}, mp::ModelParameters{N, M}, x_seq) where {N, M}
    update_pmf_z1!(hs, mp, x_seq)
    update_A!(hs, mp, x_seq)
    update_prop_list!(hs, mp, x_seq)
end
